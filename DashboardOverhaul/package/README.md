...
- **侧栏**：使用游戏原生箭头开合侧栏。背景与网格保持原位，图表层平移避让；页签位于图表及侧栏菜单上方。/ **Sidebar**: use the game's native arrow to toggle it. The background and grid stay in place while the chart layer moves clear; tabs remain above charts and sidebar menus.
