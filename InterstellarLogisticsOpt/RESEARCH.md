...
This source review includes executable offline witnesses. Subsequent [game capture results](PROFILING.md) identify measured hotspots and comparison limits; they do not establish routing or throughput acceptance. These findings describe ILO 1.1.0. The current implementation retains required dispatch side effects and the station-ID phase assignment from 1.1.0. Cross-station priority differences are an accepted tradeoff; lock aging and return loading remain native. UI/config events are unsubscribed on destruction. Current behavior and supported contracts are documented in [the package README](package/README.md); implementation validation is recorded in [PROFILING.md](PROFILING.md). The diagnostic implementation and run instructions are in [LogisticsProfiler](../LogisticsProfiler/README.md).
...
The current implementation accepts this priority tradeoff to retain station-ID phase dispersion. It does not lengthen locks or restrict returning ships. Preserving strict native cross-station priority order would require a different coordination scheme; simply multiplying lock lengths is insufficient, and `StationPriorityLock.lockTick` is a serialized byte. `60 * 5 = 300` already exceeds it.
