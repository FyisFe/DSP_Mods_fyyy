...
- **Post-review hardening (commit `668e313`):** drag handlers ignore non-left pointer buttons (a right/middle drag no longer reorders or swallows the context menu); `Refresh()`/`Free()` cancel any in-progress drag and `BeginDrag` allows only one drag at a time (no dangling-ref `MissingReferenceException`, no orphaned lifted tab); `ReorderPages` returns `bool` and rejects non-permutations up front (`newOrder.Count != active` + empty-set guards close a duplicate-slot hole and a latent sim-loop NPE), and `EndDrag` logs a warning on rejection; the per-move reflow caches the `HorizontalLayoutGroup`, reuses scratch buffers, and runs in a single pass (no per-frame allocations).
- **Auto-name blanking (commit `39213c8`):** a Harmony postfix on `DashboardLayout.AddPage` (`DashboardLayoutPatch.AddPage_Postfix`) clears the vanilla creation-index name, so an unnamed page renders its LIVE slot index — which stays correct after a reorder instead of a frozen number that could collide with another page's live index (two "5"s). Covers `Init` (first page), `SetViewPage` auto-create, and the `+` button; a player-set name is kept verbatim; not retroactive to already-saved pages.
